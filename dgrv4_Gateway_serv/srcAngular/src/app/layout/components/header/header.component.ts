...
export class HeaderComponent implements OnInit {
    ...
    constructor(
        ...
        this.aliveSec = sessionStorage.getItem('expires_in')? Number(sessionStorage.getItem('expires_in')): undefined;
    }
        ...
        if(this.idTokenJwtstr!='')
        {
          ...
          if(strToken!='')
          {
            ...
            if(jsonToken){
              ...
              if(jsonToken['picture'] && jsonToken['picture'] != '') this.userImg = jsonToken['picture'];
              this.userAlias = jsonToken['name'] ? jsonToken['name'] : jsonToken['email'] ?  jsonToken['email']: jsonToken['sub'] ? jsonToken['sub'] : this.userAlias;
    ...
    expiredProcess() {
      ...
      this.timeInterval = setInterval(() => {
          ...
          const diffSecs =  Math.round((dayjs(nowTimestamp).diff(dayjs(this.lastTimestamp), 'millisecond') / 1000));

          if (this.aliveSec && (this.aliveSec == 0 || diffSecs >= Number(sessionStorage.getItem('expires_in')))) {
          ...
          this.minStr = ('0' + Math.floor(this.aliveSec! / 60).toString()).slice(-2);
    ...
    redirect(urlParam:string) {
        ...
        {
            this.router.navigateByUrl('/udpssologin?info=' + urlPool[urlPool.length-1])
        }
        else
        {
            let redirectUrl = urlParam.replace('/'+urlPool[urlPool.length-1],'login?info=' + urlPool[urlPool.length-1])
