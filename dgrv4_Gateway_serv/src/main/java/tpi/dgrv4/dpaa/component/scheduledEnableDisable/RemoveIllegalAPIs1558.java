...
@Component
public class RemoveIllegalAPIs1558 extends Handler implements ApiPublicFlagHandlerInterface {
	...
	public ApiListAndAlgorithmClassifier handle(ApiPublicFlagHandlerData data, ApiListAndAlgorithmClassifier classifier,
		...
		long apiEnableScheduledDate = data.getApiEnableScheduledDate();
		long disableScheduledDate = data.getDisableScheduledDate();

		boolean isGreater = isFirstNumberGreaterThanOrEqualSecondNumber(disableScheduledDate, apiEnableScheduledDate);

		if (TsmpDpItem.isEqualParam1(TsmpDpItem.ENABLE_FLAG_ENABLE, apiStatus) && isGreater
				&& apiEnableScheduledDate != 0) {

			throw TsmpDpAaRtnCode._1558.throwing();
...
//			return classifier;
	...
	public List<AA0303Item> getAA0303ItemRespList(List<TsmpApi> apis, String locale) {
		...
		for (TsmpApi api : apis) {
			...
			resp.setErrMsg(getErrMsg(TsmpDpAaRtnCode._1558, locale));
