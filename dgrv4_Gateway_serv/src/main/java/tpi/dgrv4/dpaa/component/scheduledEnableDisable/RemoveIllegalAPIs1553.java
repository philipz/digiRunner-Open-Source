...
@Component
public class RemoveIllegalAPIs1553 extends Handler implements ApiPublicFlagHandlerInterface {
	...
	public ApiListAndAlgorithmClassifier handle(ApiPublicFlagHandlerData data, ApiListAndAlgorithmClassifier classifier,
		...
		boolean isLess = isFirstNumberGreaterThanOrEqualSecondNumber(apiDisableScheduledDate, enableScheduledDate);

		if (TsmpDpItem.isEqualParam1(TsmpDpItem.ENABLE_FLAG_ENABLE, apiStatus) && isLess) {

			throw TsmpDpAaRtnCode._1553.throwing();
...
//			return classifier;
	...
	public List<AA0303Item> getAA0303ItemRespList(List<TsmpApi> apis, String locale) {
		...
		for (TsmpApi api : apis) {
			...
			resp.setErrMsg(getErrMsg(TsmpDpAaRtnCode._1553, locale));
