...
@Component
public class RemoveIllegalAPIs1554 extends Handler implements ApiPublicFlagHandlerInterface {
	...
	public ApiListAndAlgorithmClassifier handle(ApiPublicFlagHandlerData data, ApiListAndAlgorithmClassifier classifier,
		...
		boolean isLess = isFirstNumberGreaterThanOrEqualSecondNumber(apiEnableScheduledDate, disableScheduledDate);

		if (TsmpDpItem.isEqualParam1(TsmpDpItem.ENABLE_FLAG_DEACTIVATE, apiStatus) && isLess) {

			throw TsmpDpAaRtnCode._1554.throwing();
...
//			return classifier;
	...
	public List<AA0303Item> getAA0303ItemRespList(List<TsmpApi> apis, String locale) {
		...
		for (TsmpApi api : apis) {
			...
			resp.setErrMsg(getErrMsg(TsmpDpAaRtnCode._1554, locale));
