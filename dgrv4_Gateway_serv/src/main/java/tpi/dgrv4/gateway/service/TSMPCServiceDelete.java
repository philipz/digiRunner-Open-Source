...
public class TSMPCServiceDelete implements IApiCacheService{
	...
	public ResponseEntity<?> forwardToDelete(HttpHeaders httpHeaders, HttpServletRequest httpReq, HttpServletResponse httpRes,
		...
		try {
			...
			TsmpApiLogReq tsmpcDelDgrReqVo = getCommForwardProcService().addEsTsmpApiLogReq1(uuid, httpReq, payload, "tsmpc", aType);
			// 第一組 RDB Req
			TsmpApiLogReq tsmpcDelDgrReqVo_rdb = getCommForwardProcService().addRdbTsmpApiLogReq1(uuid, httpReq, payload, "tsmpc", aType);
			...
			if (isMockTest) {
				return mockForwardTo(httpReq, httpRes, httpHeaders, tsmpcDel_srcUrl, uuid, tokenPayload, tsmpcDelDgrReqVo, tsmpcDelDgrReqVo_rdb, cApiKeySwitch);
			} else {
				return forwardTo(httpReq, httpRes, httpHeaders, tsmpcDel_srcUrl, payload, uuid, tokenPayload, tsmpcDelDgrReqVo, tsmpcDelDgrReqVo_rdb, cApiKeySwitch);
			}
	...
	protected ResponseEntity<?> forwardTo(HttpServletRequest httpReq, HttpServletResponse httpRes,
			@RequestHeader HttpHeaders httpHeaders, String srcUrl, String reqMbody,
			String uuid, int tokenPayload, TsmpApiLogReq tsmpcDelDgrReqVo, TsmpApiLogReq tsmpcDelDgrReqVo_rdb //
		...
		String fixedCacheId = getCommForwardProcService().getFixedCacheIdByFlagStart(tsmpcDelDgrReqVo, srcUrl, reqMbody);
		if(StringUtils.hasText(autoCacheId)) {
			...
			if(apiCacheRespVo != null) {//走cache
				tsmpcDel_respObj.setRespData(apiCacheRespVo.getStatusCode(), apiCacheRespVo.getRespStr(), apiCacheRespVo.getHttpRespArray(), apiCacheRespVo.getRespHeader());
			...
			if(cacheVo != null) {//走cache
				...
				if(isUpdate) {//更新紀錄
					...
					if(tsmpcDel_respObj.statusCode < 400) {
						...
						tsmpcDel_respObj.setRespData(cacheVo.getStatusCode(), cacheVo.getRespStr(), cacheVo.getData(), cacheVo.getRespHeader());
					}
				}else {//取得cache資料
					tsmpcDel_respObj.setRespData(cacheVo.getStatusCode(), cacheVo.getRespStr(), cacheVo.getData(), cacheVo.getRespHeader());
				}
		...
		httpRes = getCommForwardProcService().getConvertResponse(tsmpcDel_respObj.respHeader, tsmpcDel_respObj.statusCode, httpRes);
	...
	public HttpRespData callback(AutoCacheParamVo vo) {
		try {
			...
			//第二組ES REQ
			TsmpApiLogReq tsmpcDelBankendReqVo = getCommForwardProcService().addEsTsmpApiLogReq2(vo.getDgrReqVo(), vo.getHeader(), vo.getSrcUrl(), vo.getReqMbody());
	...
	private HttpRespData getHttpRespData(Map<String, List<String>> header, String reqUrl, String payload) throws Exception {
		HttpRespData httpRespData = HttpUtil.httpReqByRawDataList(reqUrl, "DELETE", payload,header, true, false, maskInfo);
