...
public class TSMPCServicePut implements IApiCacheService{
	...
	public ResponseEntity<?> forwardToPut(HttpHeaders httpHeaders, HttpServletRequest httpReq, HttpServletResponse httpRes, String payload) throws Exception {
		...
		try {
			...
			TsmpApiLogReq tsmpcPutDgrReqVo = getCommForwardProcService().addEsTsmpApiLogReq1(uuid, httpReq, payload, "tsmpc", aType);
			// 第一組 RDB Req
			TsmpApiLogReq tsmpcPutDgrReqVo_rdb = getCommForwardProcService().addRdbTsmpApiLogReq1(uuid, httpReq, payload, "tsmpc", aType);
			...
			if (isMockTest) {
				return mockForwardTo(httpReq, httpRes, httpHeaders, tsmpcPut_srcUrl, uuid, tokenPayload, tsmpcPutDgrReqVo, tsmpcPutDgrReqVo_rdb, cApiKeySwitch);
			} else {
				return forwardTo(httpReq, httpRes, httpHeaders, tsmpcPut_srcUrl, payload, uuid, tokenPayload, tsmpcPutDgrReqVo, tsmpcPutDgrReqVo_rdb, cApiKeySwitch);
			}
		}catch(Exception e) {
	...
	protected ResponseEntity<?> forwardTo(HttpServletRequest httpReq, HttpServletResponse httpRes,
			...
			, TsmpApiLogReq tsmpcPutDgrReqVo, TsmpApiLogReq tsmpcPutDgrReqVo_rdb, Boolean cApiKeySwitch) throws Exception {
		...
		String fixedCacheId = getCommForwardProcService().getFixedCacheIdByFlagStart(tsmpcPutDgrReqVo, srcUrl, reqMbody);
	...
	public HttpRespData callback(AutoCacheParamVo vo) {
		try {
			...
			//第二組ES REQ
			TsmpApiLogReq tsmpcPutBankendReqVo = getCommForwardProcService().addEsTsmpApiLogReq2(vo.getDgrReqVo(), vo.getHeader(), vo.getSrcUrl(), vo.getReqMbody());
			...
			//第二組ES RESP
			getCommForwardProcService().addEsTsmpApiLogResp2(respObj, tsmpcPutBankendReqVo, contentLength);
	...
	private HttpRespData callForwardApi(Map<String, List<String>> header, HttpServletRequest httpReq,  HttpServletResponse httpRes, 
		...
		//第二組ES REQ
		TsmpApiLogReq tsmpcPutBankendReqVo = getCommForwardProcService().addEsTsmpApiLogReq2(dgrReqVo, header, srcUrl, reqMbody);
