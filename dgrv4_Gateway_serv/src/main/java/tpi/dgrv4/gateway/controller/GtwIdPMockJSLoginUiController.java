...
public class GtwIdPMockJSLoginUiController {
	...
	public ResponseEntity<?> loginUi(@RequestHeader HttpHeaders headers, 
		...
		try {
			String dgrLoginUrl = "https://localhost:8080/dgrv4/ssotoken/gtwidp/" + idPType + "/gtwlogin";
			
			// 程式中寫死 userName, userMima 表示人輸入的, 然後 302 到 gtwlogin
			// 輸入的 user
			String userName = "minitest";
			String userMima = "mini123";
			String reqUserMima = null;
			try {
				// 因使用 get 傳送, userMima 先做 AES 加密再傳
				reqUserMima = getTsmpTAEASKHelper().encrypt(userMima);
			} catch (Exception e) {
			...
			String redirectUrl = String.format(
					...
					+ "&password=%s",
					dgrLoginUrl, 
					IdPHelper.getUrlEncode(responseType), 
					...
					IdPHelper.getUrlEncode(reqUserMima)
			);
